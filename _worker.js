...
async function getSUB(api, request, 追加UA, userAgentHeader, clientIP) {
	...
	try {
		...
		// 发送订阅获取详情通知
		if (BotToken && ChatID) {
			const 成功数 = modifiedResponses.filter(r => r.status === 'fulfilled').length;
			const 失败数 = modifiedResponses.filter(r => r.status !== 'fulfilled').length;
			const 订阅详情 = `订阅源总数: ${api.length}\n成功获取: ${成功数}\n获取失败: ${失败数}\n客户端IP: ${clientIP}`;
			
			await sendMessage(`#订阅获取详情 ${FileName}`, clientIP, 
				`${订阅详情}\n<tg-spoiler>订阅源: ${api.join(', ')}</tg-spoiler>`);
		}
